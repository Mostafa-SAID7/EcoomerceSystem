      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 text-center">
        @foreach ([
          ...
          <div class="bg-white p-6 rounded-lg shadow hover:shadow-md transition" data-aos="zoom-in">
            <img src="{{ asset('images/' . $cat['image']) }}" alt="{{ $cat['title'] }}" class="h-32 w-full object-cover rounded mb-4">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden" data-aos="fade-up">
          ...
          <div class="p-4">
            ...
            <div class="flex justify-between items-center">
              ...
              <a href="{{ route('product.show', $product->slug) }}" class="text-blue-600 hover:underline text-sm">View</a>
            </div>
          </div>
        </div>
        @endforeach
    ...
    <div class="max-w-7xl mx-auto px-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {{-- @foreach ($newArrivals as $item)
          <div class="bg-white rounded-lg shadow-md overflow-hidden" data-aos="fade-up">
            <img src="{{ asset('images/' . $item->image) }}" alt="{{ $item->name }}" class="w-full h-56 object-cover">
            <div class="p-4">
              ...
              <div class="flex justify-between items-center">
                ...
                <a href="{{ route('product.show', $item->slug) }}" class="text-blue-600 hover:underline text-sm">View</a>
              </div>
            </div>
          </div>
        @endforeach --}}
      ...
      <div class="grid gap-8 md:grid-cols-3">
        @foreach ([
          ...
          <blockquote class="bg-white p-6 rounded-lg shadow" data-aos="fade-right">
            ...
            <footer class="text-sm font-semibold text-gray-800">— {{ $testimonial['author'] }}</footer>
          </blockquote>
        @endforeach
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 text-center">
        @foreach ([
          ...
          <div data-aos="zoom-in">
            <div class="text-blue-600 text-3xl mb-2" role="img" aria-label="{{ $feature['title'] }}">{{ $feature['icon'] }}</div>
